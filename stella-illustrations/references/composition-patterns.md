...
画法：左侧输入，中间 Stella IP 或怪机器处理，右侧输出，琥珀金箭头表达主流向。

### 系统局部

适合：信息来源、过滤器、数据库、渲染器、agent 系统局部。

画法：只画 3-5 个核心模块，Stella IP 参与其中一个关键动作。
...
画法：一层层盒子，不要正式金字塔；Stella IP 在旁边搬砖或搭建。

### 地图路线

适合：从想法到上线、用户路径、内容承接路径、学习路线。

画法：一条弯曲路径，少量节点，Stella IP 牵线或走路。
...
3. 让 Stella IP 承担动作：不是站旁边，而是在机器旁操作、拉线、守门、搬运、修补、称重、扶梯子、记录、把东西放进某个怪装置。任何动作都必须保留她完整的裙装身体、固定识别点与两只连续连接的淡紫线稿手。
...
不要默认打开或复刻 `assets/examples/`。这些图片只用于风格校准，例如线条密度、留白、颜色克制、Stella IP 的固定识别点与温柔气质。

除非用户明确说“照这张 / 复刻这个构图 / 用这个案例改”，否则不要直接复用这些旧构图：

- 传送带两个断点
- Stella IP 在内容机器旁操作判断杆
- Stella IP 操作漏斗分拣流量/信任/转化
- Stella IP 处理素材鱼
- Stella IP 牵承接路径
- Stella IP 拉三层信息源
- Stella IP 分别拿喇叭/搭桥/开门
- Stella IP 使用话术工具箱
- Stella IP 举牌看常见坑路径

同类主题也要换新隐喻。例如“承接路径”不一定画路线，可以画 Stella IP 把内容尾巴接到门把手；“一鱼多吃”不一定画鱼，可以画 Stella IP 把一个纸团压成几种形状。始终避免额外、黑色或断开的手。
